...
/**
 * Gestão de usuários — só admin. Papéis: admin (gestão de usuários + acesso
 * total), gestor (uso diário, sem gestão de usuários), convidado (leitura).
 */
...
/** Cria um usuário com senha aleatória — a senha só é exibida esta vez. */
export async function createUserAction(
  ...
  await db.query(
    ...
    [email, displayName, hashPassword(generatedPassword), role],
  );
...
/** Muda o papel de um usuário — admin não pode se rebaixar (evita ficar sem admin). */
export async function updateUserRoleAction(
  ...
  if (role !== 'admin') {
    const admins = await db.query<{ count: number }>(
      "SELECT count(*)::int AS count FROM app_user WHERE role = 'admin' AND id != $1",
      [userId],
    );
  ...
  await db.query('UPDATE app_user SET role = $2, updated_at = now() WHERE id = $1', [userId, role]);
  revalidatePath('/users');
  return { ok: 'Papel atualizado.' };
}

/** Remove um usuário — nunca a si mesmo, nunca o último admin. */
export async function deleteUserAction(formData: FormData): Promise<void> {
  ...
  const db = await getDb();
  const target = await db.query<{ role: AppUserRole }>('SELECT role FROM app_user WHERE id = $1', [
    userId,
  ]);
